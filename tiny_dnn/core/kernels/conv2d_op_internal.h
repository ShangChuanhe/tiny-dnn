...
inline void conv2d_op_internal(const tensor_t &in_data,
  ...
  //double macry = 1 / std::pow(2, FRACBIT); //the most accuracy for 7 bit fraction
  float_t macry = 1 / std::pow(2, FRACBIT); //the most accuracy for 7 bit fraction
                   ...
                   for (size_t wy = 0; wy < kh; wy++) {    // NOLINT
                     for (size_t wx = 0; wx < kw; wx++) {  // NOLINT
                       ...
                       */
                       ...
                       //sum += pw_element[wx] * pin_element[wx * w_dilation] + 0.01;
                     }
                     pw_element += kw;
